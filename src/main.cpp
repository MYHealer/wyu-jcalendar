...
WiFiManagerParameter para_chaoxing_ck("chaoxing_ck", "学习通CK（用于自动获取课程表）", "", 2000); //     学习通Cookie
...
void loop() {
    ...
    // 前置任务：wifi已连接
    // 获取学习通课表（有CK时每次唤醒拉取一次）
    if (WiFi.isConnected()) {
        Preferences p;
        p.begin(PREF_NAMESPACE);
        String ck = p.getString(PREF_CHAOXING_CK, "");
        p.end();
        if (ck.length() > 0) {
            chaoxing_fetch_and_save();
        }
    }
    // 获取Weather信息
    if (weather_status() == -1) {
...
void saveParamsCallback() {
    ...
    pref.putString(PREF_CHAOXING_CK, para_chaoxing_ck.getValue());
    pref.end();
...
void buttonDoubleClick(void* oneButton) {
    ...
    String chaoxingCk = pref.getString(PREF_CHAOXING_CK);
    pref.end();
    ...
    para_chaoxing_ck.setValue(chaoxingCk.c_str(), 2000);
    ...
    wm.addParameter(&para_chaoxing_ck);
