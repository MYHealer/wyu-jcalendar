...
void drawStudySchedule() {
    ...
    for (int x = 0; x < daysCount; x++) {
        ...
        do {
            ...
            ss.trim();
            // Serial.printf("%s\r\n", ss.c_str());
            // 课名超过12个字符时截断为11个字符+省略号
            {
                int charCount = 0;
                int byteIdx = 0;
                int lastByteIdx = 0; // 第11个字符的结束位置
                while (byteIdx < ss.length() && charCount < 12) {
                    unsigned char c = ss[byteIdx];
                    int charLen = 1;
                    if ((c & 0x80) == 0x00) charLen = 1;
                    else if ((c & 0xE0) == 0xC0) charLen = 2;
                    else if ((c & 0xF0) == 0xE0) charLen = 3;
                    else if ((c & 0xF8) == 0xF0) charLen = 4;
                    if (charCount < 11) lastByteIdx = byteIdx + charLen;
                    byteIdx += charLen;
                    charCount++;
                }
                if (byteIdx < ss.length()) {
                    ss = ss.substring(0, lastByteIdx) + "...";
                }
            }
                        if (yStep == 0) {
                ...
                u8g2Fonts.setForegroundColor(fontColor);
                                                                                ...
                                                                                // 自适应字体+自动换行：逐级尝试，保证文字完整显示
                int availW = cellWidth - 4; // 留2px左右边距
                // 格子顶部Y坐标（基准点）
                int cellTop = calLayout.topH + calLayout.headerH + cellHeight * (yStep - 1) + 2 * segmentIndex;
                int len = ss.length();

                // 辅助函数：按像素宽度找断行点，返回断点字节位置数组
                // lines: 输出每行字符串数组, maxLines: 最多行数
                auto splitToLines = [&](const String& text, int maxLines, String* lines, int& lineCount) {
                    lineCount = 0;
                    ...
                    for (int line = 0; line < maxLines && remaining > 0; line++) {
                        ...
                        // 从后往前找断行点
                        int bestBreak = pos + remaining;
                        for (int ci = pos + remaining - 1; ci > pos; ci--) {
                            if ((text[ci] & 0xC0) == 0x80) continue; // 跳过续字节
                            String sub = text.substring(pos, ci);
                            if (u8g2Fonts.getUTF8Width(sub.c_str()) <= availW) {
                                bestBreak = ci;
                                break;
                            }
                        }
                        lines[lineCount++] = text.substring(pos, bestBreak);
                        remaining -= (bestBreak - pos);
                        pos = bestBreak;
                    }
                };

                // 尝试策略：16px单行 → 12px单行 → 12px双行 → 12px三行 → 截断省略号
                bool drawn = false;

                // 策略1: 16px单行
                u8g2Fonts.setFont(FONT_TEXT);
                int textW = u8g2Fonts.getUTF8Width(ss.c_str());
                if (textW <= availW) {
                    int drawY = cellTop + (cellHeight + 16) / 2 - 3;
                    u8g2Fonts.drawUTF8(cellWidth * x + marginLeft + (availW - textW) / 2, drawY, ss.c_str());
                    drawn = true;
                }
                ...
                if (!drawn) {
                    u8g2Fonts.setFont(FONT_SUB);
                    textW = u8g2Fonts.getUTF8Width(ss.c_str());
                    if (textW <= availW) {
                        int drawY = cellTop + (cellHeight + 12) / 2 - 2;
                        u8g2Fonts.drawUTF8(cellWidth * x + marginLeft + (availW - textW) / 2, drawY, ss.c_str());
                        drawn = true;
                    }
                }
                ...
                if (!drawn) {
                    ...
                    int lineCount = 0;
                    // 先尝试2行
                    splitToLines(ss, 2, lines, lineCount);
                    bool twoLineOk = (lineCount == 2);
                    if (twoLineOk) {
                        for (int li = 0; li < lineCount; li++) {
                            if (u8g2Fonts.getUTF8Width(lines[li].c_str()) > availW) {
                                twoLineOk = false;
                                break;
                            }
                        }
                    ...
                    if (!twoLineOk && cellHeight >= 40) {
                        // 2行塞不下，尝试3行
                        splitToLines(ss, 3, lines, lineCount);
                    }

                                        if (lineCount >= 2) {
                        int fontH = 12;
                        int lineSpacing = 2;
                        int totalH = fontH * lineCount + lineSpacing * (lineCount - 1);
                        int textTop = cellTop + (cellHeight - totalH) / 2;
                        int ascent = 10;
                        for (int li = 0; li < lineCount; li++) {
                            int lw = u8g2Fonts.getUTF8Width(lines[li].c_str());
                            // 用格子实际宽度居中，而非availW
                            int lx = (lw < cellWidth) ? (cellWidth - lw) / 2 : 2;
                            int ly = textTop + li * (fontH + lineSpacing) + ascent;
                            u8g2Fonts.drawUTF8(cellWidth * x + marginLeft + lx, ly, lines[li].c_str());
                        }
                        drawn = true;
                    }
                }
                ...
                if (!drawn) {
                    u8g2Fonts.setFont(FONT_SUB);
                    String drawStr = ss;
                    while (drawStr.length() > 1) {
                        int lastIdx = drawStr.length() - 1;
                        while (lastIdx > 0 && (drawStr[lastIdx] & 0xC0) == 0x80) lastIdx--;
                        drawStr = drawStr.substring(0, lastIdx);
                        textW = u8g2Fonts.getUTF8Width((drawStr + "..").c_str());
                        if (textW <= availW) {
                            drawStr = drawStr + "..";
                            break;
                        }
                    ...
                    int drawY = cellTop + (cellHeight + 12) / 2 - 2;
                                        u8g2Fonts.drawUTF8(cellWidth * x + marginLeft + (availW - u8g2Fonts.getUTF8Width(drawStr.c_str())) / 2, drawY, drawStr.c_str());
                }
            }
