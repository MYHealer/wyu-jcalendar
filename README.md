...
4.2 寸三色墨水屏日历，支持农历、公共假期、倒计日、天气、课程表展示。项目以低难度、低成本的方式，方便爱好者实现属于自己的低功耗月历。

**本魔改版新增学习通（超星）课程表自动获取功能**，适用于五邑大学学生查看课程安排；其他使用学习通的高校也可尝试使用。

<img src="./assets/img/sample.jpg" width="60%"><br>
<img src="./assets/img/sample2.jpg" width="60%">

## 新增功能：学习通课程表

在原版手动输入课程表的基础上，新增**学习通 CK 自动获取**：

- 在配置页面填入学习通 Cookie（CK），设备每次唤醒时自动调用学习通 API 拉取本周课程
- 无需手动逐条输入课程，省时省力
- 与原版手动课程表配置共存，未填 CK 时仍可使用手动模式
...
4. **学习通 CK**：复制粘贴学习通 Cookie（CK），自动调用学习通 API 获取本周课程。
...
此项目使用 GNU General Public License v3.0 许可证授权。详情请参阅 LICENSE 文件。
...
Original project: [JADE-Jerry/jcalendar](https://github.com/JADE-Jerry/jcalendar)
